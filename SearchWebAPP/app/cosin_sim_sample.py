...
class OverviewSearch:
    ...
    def search_top_n_similar_overviews(self, question, top_n=3):
        ...
        top_n_scores = np.sort(similarities[0])[-top_n:][::-1]  # スコアを取得
        return [(self.entries[i], top_n_scores[idx]) for idx, i in enumerate(top_n_indices)]  # スコアも返す

    def search_top_n_similar_overviews_old(self, question, top_n=3):
        """質問に最も類似した上位n件の概要を返す"""
        question_embedding = self.get_embedding(question)
        similarities = cosine_similarity(question_embedding, self.overview_embeddings)
...
# クラスの初期化
#overview_search = OverviewSearch(service_catalog_file, embeddings_file, use_saved_embeddings=False)
