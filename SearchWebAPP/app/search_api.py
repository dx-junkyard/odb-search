...
def label_text(text: str) -> dict[str, list[str]]:
    """
    LLMHtmlConverter.generate_json_from_question で返される JSON を
    { '対象者ラベル': [...], 'サービスラベル': [...] } の形式で返す。
    情報が不足している場合は、追加の質問を含む辞書を返す。
    """
    try:
        logger.info(f"テキストのラベル付けを開始: {text[:100]}...")
        result = labeler.generate_json_from_question(text)
        logger.info(f"LLMからの応答: {result}")
        ...
        if result.get("needs_more_info", False):
            return {
                ...
                "additional_question": result.get("additional_question", "申し訳ありません。もう一度質問を入力してください。")
            }
        
        # ラベルの処理
        result.pop("needs_more_info", None)  # needs_more_infoキーを削除
        return result
...
async def search(req: SearchRequest):
    try:
        ...
        logger.info(f"ラベル付け結果: {user_labels}")

        # 情報が不足している場合は、追加の質問を返す
        if user_labels.get("needs_more_info", False):
            response = {
                "needs_more_info": True,
                "additional_question": user_labels.get("additional_question"),
                "results": []
            }
            logger.info(f"追加の質問を返却: {response}")
            return response
